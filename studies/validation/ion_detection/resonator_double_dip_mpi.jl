...
for z_start in z_start_local
    ...
    trap = IdealTrap(-49.63, -14960.0, 1.0)
    ...
    sim = Simulation(setup, dt=2*pi/omega_p(trap, species)/OVERSAMPLING, stop_time=N_AXIAL_CYCLES*2*pi/omega_z(trap, species))
    sim.output_writers[:resistor_v] = MemoryWriter(CircuitObservable(resistor.v), TimeInterval(2*pi/omega_z(trap, species)/SAVE_OVERSAMPLING))
...
if rank == ROOT
    ...
    savefig(joinpath(@__DIR__, "resonator_double_dip_mpi.png"))
