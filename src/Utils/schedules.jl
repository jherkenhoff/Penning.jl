...
Pay attention that this schedule might not actuate in equidistant intervals if the passed `interval` is not properly 
aligned with the simulation time step. 
See [`AlignedTimeInterval`](@ref) for an aligned version.
"""
TimeInterval(interval::Number) = TimeInterval(Float64(interval), 0.0)
...
"""
function AlignedTimeInterval(interval::Number, dt::Number)
    interval < dt && error("Interval ($interval) smaller than dt ($dt) is not allowed")
    d, r = divrem(interval, dt)
    if r < dt/2
        return IterationInterval(d)
    else
        return IterationInterval(d+1)
    end
end

function (schedule::TimeInterval)(setup)
    time = setup.clock.time
...
Returns a callable `IterationInterval` that "actuates"
whenever the simulation iteration (modified by `offset`) is a multiple of `interval`.
For example, 
...
(schedule::IterationInterval)(setup) = (setup.clock.iteration - schedule.offset) % schedule.interval == 0
...
Returns a callable `WallTimeInterval` that actuates
...
function (schedule::WallTimeInterval)(setup)
...
Returns a callable `SpecifiedTimes` that actuates at predefined simulation times.
Pass times either by individual arguments `SpecifiedTimes(0.0, 3.14, 8.9)` or as
a list `SpecifiedTimes([0.0, 3.14, 8.9])`
"""
...
function (st::SpecifiedTimes)(setup)
    current_time = setup.clock.time
...
"""
ConsecutiveIterations(parent_schedule::AbstractSchedule, N::Integer=1) = ConsecutiveIterations(parent_schedule, N, 0)
