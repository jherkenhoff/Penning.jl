...
function connect_electrode_to_circuit!(c::CircuitConnection, setup::Setup)
    i = get_electrode_selection_current(c.electrode_selection, setup)
    add_circuit_pin_selection_current!(c.circuit_pin_selection, setup, i)
    nothing
end

function connect_circuit_to_electrode!(c::CircuitConnection, setup::Setup)
